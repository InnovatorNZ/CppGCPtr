...
public:
	...
	static constexpr bool enableParallelGC = false;				// 是否启用多线程回收；前提条件：启用内存分配器
	static constexpr bool enableDestructorSupport = false;		// 是否在销毁对象时调用其析构函数
	...
	static constexpr bool enableHashPool = false;				// 是否启用线程id进行hash后取模的池化方案；可以降低锁的竞争，但可能会产生计算哈希的开销
